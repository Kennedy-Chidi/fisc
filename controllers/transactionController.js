...
exports.createTransaction = catchAsync(async (req, res, next) => {
  ...
  if (data.autoTransact) {
    const wallet = await Wallet.findOne({
      currencyId: data.walletId,
      username: data.username,
    });
    if (!wallet) {
      return next(
        new AppError(`Please select wallet to make transaction`, 404)
      );
    }
    ...
    data.planDuration = plan.planDuration * 24 * 60 * 60 * 1000;
    ...
    sendTransactionEmail(data.user, `deposit-approval`, req.body.amount, next);
